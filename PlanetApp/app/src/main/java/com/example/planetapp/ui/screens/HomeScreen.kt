...
fun HomeScreen(
    ...
    val planetStateList = remember { mutableStateListOf(*planetList.toTypedArray()) }

    val filteredPlanets by remember(searchQuery, planetStateList) {
        derivedStateOf {
            planetStateList.filter { it.name.contains(searchQuery, ignoreCase = true) }
        }
    }
    ...
    fun toggleFavorite(planet: Planet) {
        val index = planetStateList.indexOfFirst { it.id == planet.id }
        if (index != -1) {
            planetStateList[index] = planet.copy(isFavorite = !planet.isFavorite)
            planetList[index] = planet.copy(isFavorite = !planet.isFavorite)
        }
    }
    ...
    Scaffold(
        ...
        Column(modifier = Modifier.padding(innerPadding)) {
            ...
            // Listagem de planetas filtrados
            LazyColumn(
